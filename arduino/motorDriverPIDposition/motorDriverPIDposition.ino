...
void loop() {
  ...
  set_pos = map(rotary_count, 0, 100, 0, 255) * 2;
  ...
  if(millis() > serial_time + 10) {// Every 100 ms
    ...
    degrees = encoder_count * (360.0 / 748.0);

    // Proportional controller
    Kp = 0.4; // adjust this experimentally
    error = set_pos - degrees;
    Ki = 0;
    integral += error * dt;
    ...
    if (error < -1) {
      digitalWrite(DIR_B, HIGH);
      motor_command = abs(Kp * error + Ki * integral + Kd * derivative);
    } else if (error > 1) {
      digitalWrite(DIR_B, LOW);
